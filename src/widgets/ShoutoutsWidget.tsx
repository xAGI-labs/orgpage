      ...
      <div className="grid gap-4 md:grid-cols-[1fr_0.9fr]">
        <div className="grid gap-3">
          {bundle.shoutouts.length === 0 ? (
            <p className="rounded-md bg-[var(--brand-soft)] p-4 text-sm font-semibold text-[var(--muted)]">Be the first to give kudos.</p>
          ) : (
        ...
        <form onSubmit={submit} className="rounded-md bg-[var(--brand-soft)] p-4">
          ...
          <Button className="mt-3 w-full" loading={busy} icon={<Send className="h-4 w-4" />}>
            Send kudos
          </Button>
        </form>
      </div>
